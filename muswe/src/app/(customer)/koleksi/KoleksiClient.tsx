...
export function KoleksiClient({ collections }: KoleksiClientProps): React.JSX.Element {
  const { isEnglish } = useTranslation()

  return (
    <div className="bg-white min-h-screen">
      <PageHero
        eyebrow={isEnglish ? 'Collections List' : 'Daftar Koleksi'}
        title={isEnglish ? 'Special Collections' : 'Koleksi Spesial'}
        subtitle={
          isEnglish
            ? 'Explore various curated premium product collections from Muswe.'
            : 'Jelajahi berbagai edisi dan koleksi produk kurasi premium dari Muswe.'
        }
        variant="dark"
      />
          ...
          {collections.map((col) => (
            ...
            >
              ...
              <div className="absolute top-4 left-4 px-3 py-1.5 bg-brand-gold/90 backdrop-blur-sm">
                <span className="text-sm font-heading font-bold uppercase tracking-wider text-brand-black">
                  {isEnglish ? 'Collection' : 'Koleksi'}
                </span>
              </div>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end p-6 md:p-8 text-white text-center space-y-2 z-10">
                ...
                <span className="text-sm font-heading uppercase tracking-[0.1em] text-brand-gold-light pt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  {isEnglish ? 'View Collection' : 'Lihat Koleksi'} &rarr;
                </span>
              </div>
            </Link>
          ))}
