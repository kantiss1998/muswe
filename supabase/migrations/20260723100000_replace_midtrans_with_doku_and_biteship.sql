-- =============================================================
-- Migration: 20260723100000_replace_midtrans_with_doku_and_biteship.sql
-- Description:
-- 1. Rename Midtrans payment fields to generic gateway fields & add DOKU fields
-- 2. Add Biteship tracking fields to order_shipping
-- 3. Drop legacy manual shipping zone/rate tables
-- 4. Update create_order RPC function to use gateway_order_id
-- =============================================================
...
ALTER TABLE payments ADD COLUMN IF NOT EXISTS doku_session_id VARCHAR(200);
...
DROP INDEX IF EXISTS idx_payment_logs_midtrans_order_id;
CREATE INDEX IF NOT EXISTS idx_payment_logs_gateway_order_id ON payment_logs(gateway_order_id);

COMMENT ON TABLE payments IS 'Payment records linked to payment gateway (DOKU)';
COMMENT ON TABLE payment_logs IS 'Audit log of gateway webhook events for idempotency';

-- 2. Update order_shipping table for Biteship
ALTER TABLE order_shipping ADD COLUMN IF NOT EXISTS courier_code VARCHAR(50);
ALTER TABLE order_shipping ADD COLUMN IF NOT EXISTS biteship_order_id VARCHAR(100);
ALTER TABLE order_shipping ADD COLUMN IF NOT EXISTS courier_service_name VARCHAR(100);

-- 3. Drop legacy manual shipping tables & RPCs as requested
DROP TABLE IF EXISTS shipping_rates CASCADE;
DROP TABLE IF EXISTS shipping_zone_coverage CASCADE;
DROP TABLE IF EXISTS shipping_zones CASCADE;

DROP FUNCTION IF EXISTS calculate_shipping(UUID, INT);
DROP FUNCTION IF EXISTS admin_create_shipping_zone(JSONB, TEXT[]);
DROP FUNCTION IF EXISTS admin_update_shipping_zone(UUID, JSONB, TEXT[]);

-- 4. Update create_order RPC function
