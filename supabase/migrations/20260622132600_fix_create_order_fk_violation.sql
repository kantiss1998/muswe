-- Migration: 20260622132600_fix_create_order_fk_violation.sql
-- Fix foreign key constraint violation in create_order by inserting the order record before the order items.
...
COMMENT ON FUNCTION public.create_order IS 'Atomic checkout: validate cart, voucher, shipping → create order + deduct stock + clear cart with enhanced error logs';
